...
zoo`.split("\n");
  ...
  function validateEvent(event) {
    if (typeof event.content !== "string") {console.log("error here");return false;}
    if (typeof event.created_at !== "number") {console.log("error here");return false;}
    if (typeof event.pubkey !== "string") {console.log("error here"); console.log(event);return false;}
    if (!event.pubkey.match(/^[a-f0-9]{64}$/)) {console.log("error here");return false;}
    if (!Array.isArray(event.tags)) {console.log("error here");return false;}
    for (let i = 0; i < event.tags.length; i++) {
      let tag = event.tags[i];
      if (!Array.isArray(tag)) {console.log("error here");return false;}
      for (let j = 0; j < tag.length; j++) {
        if (typeof tag[j] === "object") {console.log("error here");return false;}
      }
    }
    return true;
  }
